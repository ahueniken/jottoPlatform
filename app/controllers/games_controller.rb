    ...
    if !@@setupCompleted
      ...
      if current_user.id == @game.user1
        if (@game.p1GuessResults != nil)
          ...
          guesses[params[:game][:p1guesses].upcase] = compareWords(params[:game][:p1guesses].upcase, @game.word2.upcase)
        else 
          guesses = Hash.new
          guesses[params[:game][:p1guesses].upcase] = compareWords(params[:game][:p1guesses].upcase, @game.word2.upcase)
        end
       ...
       if (@game.p2GuessResults != nil)
          ...
          guesses[params[:game][:p2guesses].upcase] = compareWords(params[:game][:p2guesses].upcase, @game.word1.upcase)
        else 
          guesses = Hash.new
          guesses[params[:game][:p2guesses].upcase] = compareWords(params[:game][:p2guesses].upcase, @game.word1.upcase)
        end
